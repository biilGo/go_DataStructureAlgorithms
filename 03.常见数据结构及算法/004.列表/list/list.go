...
// AddNodeFromHead从头部开始，添加节点到第N+1个元素之前，N=0表示添加到第一个元素之前，表示新节点成为新的头部，N=1表示添加到第二个元素之前，以此类推
func (list *DoubleList) AddNodeFromHead(n int, v string) {
	...
	if node.IsNil() {
		...
		// 如果定位的节点前驱为nil，那么定位到的节点为链表头部，需要换头部
		if pre.IsNil() {
			// 将新节点链接在老头部之前
			newNode.next = node
			node.pre = newNode
...
// AddNodeFromTail从尾部开始，添加节点到第N+1个元素之后，N=0表示添加到第一个元素之后，表示新节点成为新的尾部，N=1表示添加到第二个元素之后，以此类推
func (list *DoubleList) AddNodeFromTail(n int, v string) {
	...
	if node.IsNil() {
		...
		// 如果定位到节点后驱为nil，那么定位到的节点为链表尾部，需要换尾部
		if next.IsNil() {
			// 将新节点链接在老尾部之后
			node.next = newNode
			newNode.pre = node
...
// IndexFromHead从头部开始往后找，获取第N+1个位置的节点，索引从0开始
...
// IndextFromTail从尾部开始往前找，获取第N+1个位置的节点，索引从0开始
func (list *DoubleList) IndextFromTail(n int) *ListNode {
	// 索引超过或等于列表长度，一定找不到，返回空指针
	if n >= list.len {
		return nil
	}
...
// PopFromHead从头部开始往后找，获取第N+1个位置的节点，并移除返回
