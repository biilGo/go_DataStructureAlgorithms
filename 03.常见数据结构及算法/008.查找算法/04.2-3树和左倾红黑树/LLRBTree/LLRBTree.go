...
// 红色左移
// 节点h是红节点.其左儿子和左儿子的左儿子都为黑节点,左移后使得其左儿子或左儿子的左儿子有一个红色节点
...
// 红色右移
// 节点h是红节点,其右儿子和右儿子的左儿子都为黑节点,右移使得其右儿子或右儿子的右儿子有一个红色节点
func MoveRedRight(h *LLRBTNode) *LLRBTNode {
	// 应该确保isRed(h)&&!isRed(h.right)&&!isRed(h.right.left)
	ColorChange(h)
...
func (node *LLRBTNode) Add(value int64) *LLRBTNode {
	...
	// 这里做完操作就可以结束了,因为插入操作,新插入的右红链接左旋后,nowNode节点不会出现连续两个红左链接,因为它只有一个左红链接
...
func (node *LLRBTNode) Delete(value int64) *LLRBTNode {
	...
	if value < nowNode.Value {
		...
		// 左节点为红色,那么需要右移,方便后面可以红色右移
		if IsRed(nowNode.Left) {
			nowNode = RotateRight(nowNode)
		}

		// 值相等,且没有有孩子节点,那么该节点一定是要被删除的叶子节点,直接删除
		// 为什么呢?反正,它没有右儿子,但有左儿子,因为左倾红黑树的特征,那么左儿子一定是红色,但是前面的语句已经把红色左儿子右旋到右边,不应该出现右儿子为空
		...
		if !IsRed(nowNode.Right) && !IsRed(nowNode.Right.Left) {
			// 右儿子和左儿子的左儿子都不是红色节点,那么没法递归下去,先红色右移
...
func (tree *LLRBTree) IsLLRBTree() bool {
	...
	// 判断树是否遵循2-3树,也就是红链接只能在左边,不能连续有2个红链接
	if !tree.Root.Is23() {
		return false
	}
