...
func (h *Heap) Push(x int) {
	...
	for i > 0 {
		...
		// 否则将父亲节点与该节点互换,然后向上翻转,将最大的元素一直网上堆
...
func (h *Heap) Pop() int {
	...
	for {
		...
		// 有右子树,拿到两个儿子节点中较大节点的下标
		if b < h.Size && h.Array[b] > h.Array[a] {
			a = b
		}

		// 父亲节点的值都大于或等于两个儿子较大的那个,不需要向下继续翻转,返回
		if x >= h.Array[a] {
			break
		}
