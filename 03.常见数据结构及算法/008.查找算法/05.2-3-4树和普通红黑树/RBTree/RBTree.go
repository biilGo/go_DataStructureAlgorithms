...
func (tree *RBTree) Add(value int64) {
	...
	// 辅助变量t,表示新元素要插入到该子树,t是该子树的根节点
	t := tree.Root

	// 插入元素后,插入元素的父亲节点
	var parent *RBTNode

	// 辅助变量,为了直到元素最后要插到左边还是右边
	var cmp int64 = 0
	...
	for {
		...
		// 终于找到要插入的位置了
		if t == nil {
			break // 这时叶子节点是parent,要插入到parent的下面,跳到外层去
		}
	}
...
func (tree *RBTree) fixAfterInsertion(node *RBTNode) {
	...
	for node != nil && node != tree.Root && node.Parent.Color == RED {
		...
		if ParentOf(node) == LeftOf(ParentOf(ParentOf(node))) {
			...
			// 图例3右边部分,叔叔是红节点祖父变色,也就是父亲和叔叔变黑,祖父变红
			if IsRed(uncle) {
				SetColor(ParentOf(node), BLACK)
				SetColor(uncle, BLACK)
				SetColor(ParentOf(ParentOf(node)), RED)
...
// 删除节点核心函数
// 找到最小后驱节点来补位,删除内部节点转为删除叶子节点
func (tree *RBTree) delete(node *RBTNode) {
	// 如果左右子树都存在,那么从右子树的左边一直找一直找,就能找到最小的后驱节点
	...
	if node.Left == nil && node.Right == nil {
		...
		if node.Parent == nil {
			// 要删除的节点的父亲为空,表示要删除的节点为很节点,唯一子节点成为树根
...
// 调整删除的叶子节点,自底向上
// 看图理解
func (tree *RBTree) fixAfterDeletetion(node *RBTNode) {
	// 如果不是递归到根节点,且节点是黑节点,那么继续递归
	for tree.Root != nil && !IsRed(node) {
		// 要删除的节点在父亲左边,对应图例1,2
		if node == LeftOf(ParentOf(node)) {
			// 找出兄弟
			...
			// 兄弟是黑色的,对应图例21,22,23
			// 兄弟的左右儿子都是黑色,进入图例23,将兄弟设为红色人,父亲所在的子树作为整体,当作删除的节点,继续向上递归
			if !IsRed(LeftOf(brother)) && !IsRed(RightOf(brother)) {
				SetColor(brother, RED)
				node = ParentOf(node)
			} else {
				// 兄弟的右儿子是黑色的,进入图例22,将兄弟设为红色,兄弟的左儿子设为黑色,对兄弟右旋,进入图例21
				...
				// 兄弟的右儿子是红色,进入图例21,将兄弟设为父亲的颜色,兄弟的右儿子以及父亲黑,对父亲左旋
			...
			// 兄弟是红色,对应图例3,那么兄弟变黑,父亲变红,然后对父亲右旋,进入图例41,42,43
			if !IsRed(brother) {
				SetColor(brother, BLACK)
				SetColor(ParentOf(node), RED)
			...
			// 兄弟是黑色,对应图例41,42,43
			// 兄弟的左右儿子都是黑色,进入图例43将兄弟设为红色,父亲所在的子树作为整体,当作删除的系欸但,继续向上递归
			if !IsRed(LeftOf(brother)) && !IsRed(RightOf(brother)) {
				SetColor(brother, RED)
				node = ParentOf(node)
			} else {
				// 兄弟的左儿子是黑色,键入图例42,将兄弟设为红色, 兄弟的右儿子设为黑色,对兄弟左旋,进入图例41
